...
# Mainnet Splitfund Script (autosplit-new-nnsplit.sh)
# Copyright (c) 2018-2022 Decker

# This Source Code Form is subject to the terms of the Mozilla Public
# License, v. 2.0. If a copy of the MPL was not distributed with this
# file, You can obtain one at https://mozilla.org/MPL/2.0/.

komodo_cli=$HOME/KomodoOcean/src/komodo-cli
...
function do_autosplit() {
    ...
    utxo=$($komodo_cli $asset nn_getwalletinfo | jq '.notaryvins_utxos_count')
    ...
    if [ -n "$utxo" ] && [ "$utxo" -eq "$utxo" ] 2>/dev/null; then
        ...
        #     need=1
            ...
            log_print "${DARKGREY}$komodo_cli $asset nn_split ${need}${RESET}"
            ...
            # { "tx": "e009c707d19b5327b63ff1cb6a406411dd6a1610b8cfe2a3b25e0a0a89e0a2aa", "input_utxos_value": 0.99960000, "input_utxos_count": 1, "out_notaryvins_count": 7, "out_utxos_value": 0.99880000, "out_utxos_count": 1, "estimated_tx_size": 348, "real_tx_size": 499 }
            splitres=$($komodo_cli $asset nn_split ${need})
            out_notaryvins_count=$(echo $splitres | jq -r .out_notaryvins_count)
            txid=$(echo $splitres | jq -r .tx)
            
            #if [ -z "$error" ] || [ "$error" = "null" ] && [ ! -z "$splitres" ]; then
            if [ -n "$out_notaryvins_count" ] && [ "$out_notaryvins_count" -eq "$need" ] 2>/dev/null; then
